...
This minor folds the Lane A quick-wins, the Lane B correctness/security patch, the Lane C observability + crypto-rotation + ASGI work, and the Lane D hardening subset into a single release (rather than splitting v0.1.1 / v0.2.0 / v0.3.0). Surface is additive — v0.1.0 users upgrade with a pin bump. Deferred to a later minor: naming a co-maintainer (#D3), the announcement post (#D11), and the ecosystem adapters (Flask #D7, Celery #D8, `doctor` CLI #D9, hosted docs site #D10).

### Added

- Hypothesis property-based tests for the circuit-breaker state machine — a `RuleBasedStateMachine` driving randomised CLOSED/OPEN/HALF_OPEN action sequences against a `FakeClock`, asserting the LLD §4 invariants (excluded exceptions never transition; `fail_max` ⇒ OPEN; past `reset_timeout` ⇒ admitted; `success_threshold` ⇒ CLOSED). Exercises the `hypothesis` dev dependency that had shipped unused. Test-only. (Lane D #D4)
- `[sentry]` extra with a `SentryMetricsSink` (`RESILIENCE_METRICS_SINK=sentry`) that leaves a Sentry breadcrumb for the kit's significant failure signals (`breaker.open`, `retry.exhausted`, `throttle.fail_closed`, …), carrying `request_id`/`correlation_id` + tags in the breadcrumb data — so a later captured exception shows the resilience trail that preceded it. `docs/sentry-integration.md` covers the sink and the request-id-as-tag middleware recipe. (Lane C #C4)
- `[otel]` extra with OpenTelemetry support: an `OtelMetricsSink` (select `RESILIENCE_METRICS_SINK=otel`) mapping kit metrics to the OTel meter API under `resilience_kit.*`, and a `resilience_kit.tracing` package with `TracingMiddleware` (opens a W3C-context SERVER span per request, tags it with `request_id`/`correlation_id`, marks 5xx as ERROR) plus `inject_trace_context`. `AsyncAPIClient` now injects the active `traceparent` into outbound requests best-effort, so inbound→outbound hops form one connected trace — a no-op when `[otel]` is not installed. `TracingMiddleware` is opt-in (place it inside `RequestIdMiddleware`). New ADR-0016 + `docs/otel-integration.md`. (Lane C #C3)
...
- Redis client ownership is centralised: cache, breaker, and throttle now share **one** process-wide `redis.asyncio.Redis` per URL via `resilience_kit._redis.get_redis_client`, instead of each provider calling `Redis.from_url` on first build. The shared client is closed explicitly on adapter shutdown (FastAPI lifespan exit, Django daemon-loop teardown) and on async test reset (`reset_all_singletons_async` → `aclose_redis_clients`), rather than leaked to GC on re-registration. Removes the three-independent-pools overhead. New ADR-0017. (Lane D #D1)
...
- Sync `@circuit_breaker` no longer raises `RuntimeError: <Lock ...> is bound to a different event loop` on its second consecutive call. The sync wrapper runs a fresh `asyncio.run` (a new loop) per call, but the persisted breaker's `asyncio.Lock` (in-memory state lock + the Redis backend's `_sha_lock`) stayed bound to the first, now-dead loop. Both locks are now (re)created per running loop, mirroring the loop-rebind pattern already used in `recovery.py`. (Lane D #D2)
...
- Signed release distributions (Sigstore): the PyPI publish step now sets `attestations: true`, attaching a Sigstore-signed SLSA build-provenance attestation — generated from the release workflow's OIDC identity, no long-lived key — to every wheel and sdist. Verifiable with `sigstore verify`; SLSA-3 supply-chain baseline. See `SECURITY.md`. (Lane D #D6)
- CycloneDX SBOM generated at release time and attached to each GitHub Release as `sbom-cyclonedx.json` + `sbom-cyclonedx.xml` — the dependency inventory NIST SP 800-218 / US Executive Order 14028 procurement requires. Scanned from the freshly-installed wheel's environment, so it reflects exactly what a default install pulls in. (Lane D #D5)
